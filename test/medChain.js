...
contract("MedChain", function(accounts) {
    ...
    it("it initializes the providers with the correct values", function() {
        return MedChain.deployed().then(function(instance) {
            ...
            assert.equal(provider[2], 0, "contains the correct authentication count");
    ...
    it("allows an authenticator to authenticate", function() {
        return MedChain.deployed().then(function(instance) {
            MedChainInstance = instance;
            providerId = 1;
            ...
            assert.equal(authenticateCount, 1, "increments the provider's authentication count");
        })
    });
    ...
    it("throws an exception for invalid providers", function() {
        return MedChain.deployed().then(function(instance) {
            ...
            assert.equal(authenticateCount, 1, "Provider 1 did not receive any authentication");
            return MedChainInstance.providers(2);
        }).then(function(provider2) {
            var authenticateCount = provider2[2];
            assert.equal(authenticateCount, 0, "Provider 2 did not receive any authentication");
        });
    });
});